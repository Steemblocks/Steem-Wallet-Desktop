...
const fetchPowerMeterData = async (username: string): Promise<PowerMeterData | null> => {
  try {
    ...
    const vestingSharesRaw = account.vesting_shares?.nai === '@@000000037' 
      ? BigInt(account.vesting_shares.amount)
      : BigInt(Math.floor(parseFloat(account.vesting_shares || '0') * 1e6));
    const receivedVestingRaw = account.received_vesting_shares?.nai === '@@000000037'
      ? BigInt(account.received_vesting_shares.amount)
      : BigInt(Math.floor(parseFloat(account.received_vesting_shares || '0') * 1e6));
    const delegatedVestingRaw = account.delegated_vesting_shares?.nai === '@@000000037'
      ? BigInt(account.delegated_vesting_shares.amount)
      : BigInt(Math.floor(parseFloat(account.delegated_vesting_shares || '0') * 1e6));
    const vestingWithdrawRateRaw = account.vesting_withdraw_rate?.nai === '@@000000037'
      ? BigInt(account.vesting_withdraw_rate.amount)
      : BigInt(Math.floor(parseFloat(account.vesting_withdraw_rate || '0') * 1e6));

    // Total effective vests
    const effectiveVestsRaw = vestingSharesRaw + receivedVestingRaw - delegatedVestingRaw - vestingWithdrawRateRaw;
    ...
    const currentVotingPower = maxVotingMana > BigInt(0) 
    ...
    const currentDownvotePower = maxDownvoteMana > BigInt(0) 
      ? Number((currentDownvoteMana * BigInt(10000)) / maxDownvoteMana) / 100 
      : 0;

    // Fetch RC data
    const rcResponse = await fetch('https://api.steemit.com', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        jsonrpc: '2.0',
    ...
    const rewardBalance = parseFloat(rewardFundResponse.result?.reward_balance || '0');
    const recentClaims = parseFloat(rewardFundResponse.result?.recent_claims || '1');
    ...
    try {
      ...
      console.warn('Failed to fetch blockchain median price for vote value calculation');
    }
...
export const WalletDataProvider: React.FC<WalletDataProviderProps> = ({ children }) => {
  ...
  const fetchAllData = useCallback(async (username: string, isRefresh = false) => {
    if (!username) {
    ...
    try {
      ...
      const walletData = await formatWalletDataFromAccount(account, priceData, steemPerMvests);
      setLoadingProgress(40);

      // Stage 3: Fetch delegations and witnesses in parallel (60%)
      setLoadingStage('Loading delegations & witnesses...');
      
      const [delegationsResult, witnessesResult, marketDataResult] = await Promise.all([
        steemApi.getVestingDelegations(username).catch(() => []),
        steemApi.getWitnessesByVote(null, 150).catch(() => []),
        steemApi.getSimplifiedMarketData().catch(() => null)
      ]);
      ...
      const totalDelegatedOut = outgoingDelegations.reduce(
        (sum: number, d: any) => sum + parseFloat(d.steemPower), 0
      );
      ...
      setLoadingStage('Loading market & power data...');

      // Stage 4: Fetch market history, account history, and power meter data (90%)
      const [hourlyHistory, dailyHistory, accountHistory, powerMeterData] = await Promise.all([
        steemApi.getHourlyMarketHistory().catch(() => []),
        steemApi.getDailyMarketHistory().catch(() => []),
  ...
  }, [selectedAccount, isInitialLoading, fetchAllData]);
